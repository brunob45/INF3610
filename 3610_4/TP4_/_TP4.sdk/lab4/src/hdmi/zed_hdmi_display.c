...
int zed_hdmi_display_init( zed_hdmi_display_t *pDemo )
{
  ...
  // Il faut que l'écran ait fini de boot pour qu'il reçoive correctement sa configuration... On lui laisse le temps.
  xil_printf("Generating color bars... ");
  for(int w = 0; w < 20; ++w)
  {
      // Display Color Bars
	  if (w == 0) {
	    vfb_tx_stop( &(pDemo->vdma_hdmi) );
	    zed_hdmi_display_cbars( pDemo, (w) );
	    vfb_tx_start( &(pDemo->vdma_hdmi) );
	  } else {
	    zed_hdmi_display_cbars( pDemo, (w) );
	  }

	 // Reconfig HDMI periph.
	 for (int i = 0; i < CARRIER_HDMI_OUT_CONFIG_LEN; i++ )
	 {
	    //xil_printf( "[ZedBoard HDMI] IIC Write - Device = 0x%02X, Address = 0x%02X, Data = 0x%02X\n\r", carrier_hdmi_out_config[i][0]<<1, carrier_hdmi_out_config[i][1], carrier_hdmi_out_config[i][2] );
	    pDemo->hdmi_out_iic.fpIicWrite( &(pDemo->hdmi_out_iic), carrier_hdmi_out_config[i][0], carrier_hdmi_out_config[i][1], &(carrier_hdmi_out_config[i][2]), 1 );
...
void zed_hdmi_display_clear( zed_hdmi_display_t *pDemo )
{
   // Clear frame stores
